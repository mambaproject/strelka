...
class ScanSuspiciousStrings(strelka.Scanner):
    """Scans files with YARA suspicious strings rules.
    ...
    Options:
        location: Location of the YARA rules file or directory.
            Defaults to '/etc/suspicious_rules/'.
    ...
    def scan(self, data, file, options, expire_at):
        location = options.get('location', '/etc/suspicious_rules/')
        ...
        try:
            if self.compiled_yara is not None:
                ...
                for match in yara_matches:
                    ...
                    self.event['strings'].append(match.strings)
